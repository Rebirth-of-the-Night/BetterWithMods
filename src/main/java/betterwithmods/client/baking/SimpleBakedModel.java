...
public class SimpleBakedModel implements IBakedModel {
    ...
    public void addQuad(EnumFacing side, BakedQuad quad) {
        quads.get(side == null ? 6 : side.ordinal()).add(quad);
    }

    public void addModel(IBakedModel model) {
        for (int i = 0; i < 7; i++) {
            quads.get(i).addAll(model.getQuads(null, i == 6 ? null : EnumFacing.byIndex(i), 0));
        }
    }

    @Override
    public List<BakedQuad> getQuads(IBlockState state, EnumFacing side, long rand) {
        return quads.get(side == null ? 6 : side.ordinal());
    }
