...
public class RotHandler {
    ...
    public static class RotInfo
    {
        ...
        {
            this(matchStack, InteractionBWA.MISC_ROT_TIME, "food", new ItemStack(ModItems.ROTTEN_FOOD));
        }
        ...
        public String getTranslationKey(ItemStack stack, long creationDate)
        {
            double rotPercent = (MINECRAFT_DATE - creationDate) / (double)spoilTime;
            int rotPercentInt = Math.min(100,(int)(rotPercent * 100.0));

            String returnKey = getLocalizationKey(stack.getTranslationKey(),rotPercentInt);
            if(returnKey == null)
    ...
    {
        addRottingItem(matchItem, InteractionBWA.MISC_ROT_TIME, "food", new ItemStack(ModItems.ROTTEN_FOOD));
    }

    public static void addRottingItem(ItemStack matchItem, long timeToRot)
    {
        addRottingItem(matchItem, timeToRot, "food", new ItemStack(ModItems.ROTTEN_FOOD));
    }
        ...
        if(isRottingItem(stack))
        {
            ...
            for(RotInfo info : rottingItems.get(stack.getItem())) {
                if(info.matches(stack)) {
                    String prefix =  I18n.format(info.getTranslationKey(stack, timeOfCreation));
                    if(prefix.length() > 0)
                        prefix = prefix + " ";
